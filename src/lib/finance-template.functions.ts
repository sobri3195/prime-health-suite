...
export const upsertTplInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.id) {
      ...
      await writeFinAudit({ actor_email: data.actor, action: "edit", entity: "template_invoice", entity_id: row.id, entity_no: row.nama, before, after: row });
    } else {
      ...
      if (error) throw new Error(error.message);
      row = r;
      await writeFinAudit({ actor_email: data.actor, action: "create", entity: "template_invoice", entity_id: row.id, entity_no: row.nama, after: row });
    }
...
export const upsertTplVoucher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.id) {
      ...
      await writeFinAudit({ actor_email: data.actor, action: "edit", entity: "template_voucher", entity_id: row.id, entity_no: row.nama, before, after: row });
    } else {
      ...
      if (error) throw new Error(error.message);
      row = r;
      await writeFinAudit({ actor_email: data.actor, action: "create", entity: "template_voucher", entity_id: row.id, entity_no: row.nama, after: row });
    }
    if (data.items.length) {
      const payload = data.items.map((it) => ({ template_id: row.id, ...it }));
...
export const upsertMdrRule = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.id) {
      ...
      if (error) throw new Error(error.message);
      row = r;
      await writeFinAudit({ actor_email: data.actor, action: "edit", entity: "mdr_rule", entity_id: row.id, before, after: row });
    } else {
      const { data: r, error } = await s.from("fin_mdr_rule").insert(payload).select().single();
      if (error) throw new Error(error.message);
      row = r;
      await writeFinAudit({ actor_email: data.actor, action: "create", entity: "mdr_rule", entity_id: row.id, after: row });
    }
    return { row };
  });
