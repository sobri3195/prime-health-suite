  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "/" && !typing) { e.preventDefault(); searchRef.current?.focus(); }
        ...
        const iso = (d: Date) => d.toISOString().slice(0, 10);
        setRange({ from: iso(start), to: iso(end), preset: "custom" });
        toast.success(`Rentang: 30 hari terakhir (D)`);
      }
      else if (!typing && e.key === "B") {
        e.preventDefault();
        const now = new Date();
        ...
        if (!act) { toast.info("Tidak ada action untuk disalin"); return; }
        navigator.clipboard.writeText(String(act)).then(
          () => toast.success(`Action disalin (%): ${act}`),
          () => toast.error("Gagal menyalin"),
        );
      }
      else if (!typing && e.key === "^") {
        ...
        const target = detail ?? (rows as any[])[0];
        const act = target?.action;
        if (!act) { toast.info("Tidak ada action untuk difilter"); return; }
        setAction(act);
        ...
        e.preventDefault();
        if (!(rows as any[]).length) { toast.info("Tidak ada data untuk dihitung"); return; }
        const list = rows as any[];
        const uniq = <T,>(a: T[]) => Array.from(new Set(a));
        const actors = uniq(list.map((r) => r.actor_email).filter(Boolean)).length;
        const entities = uniq(list.map((r) => r.entity).filter(Boolean)).length;
